...
const ReportDetailsModal: React.FC<ReportDetailsModalProps> = ({ report, isOpen, onClose }) => {
    ...
    const handleDownload = () => {
        ...
        doc.text("CityGuardian Report", 20, 25);

        // Report ID
        doc.setFontSize(10);
        doc.text(`ID: #${report.ID}`, 150, 25);
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text(report.Category || "Issue Report", 20, 60);

        // Status Badge styled representation
        doc.setDrawColor(37, 99, 235);
        doc.setFillColor(239, 246, 255);
        doc.roundedRect(20, 70, 40, 10, 2, 2, 'FD');
        doc.setFontSize(10);
        doc.setTextColor(37, 99, 235);
        doc.text(report.Status || "Pending", 25, 76);

        // Urgency
        const urgencyColor = report.Urgency === 'high' ? [239, 68, 68] : report.Urgency === 'medium' ? [234, 179, 8] : [59, 130, 246];
        doc.setDrawColor(urgencyColor[0], urgencyColor[1], urgencyColor[2]);
        doc.setFillColor(255, 255, 255);
        doc.roundedRect(70, 70, 40, 10, 2, 2, 'FD');
        doc.setTextColor(urgencyColor[0], urgencyColor[1], urgencyColor[2]);
        doc.text(`${report.Urgency} Priority`, 75, 76);
        ...
        const splitDescription = doc.splitTextToSize(report.issue || report.Issue || "No description provided.", 170);
        doc.text(splitDescription, 20, 110);

        const nextY = 110 + (splitDescription.length * 7) + 10;

        // Metadata
        doc.setFont('helvetica', 'bold');
        doc.text("Location:", 20, nextY);
        doc.setFont('helvetica', 'normal');
        doc.text(report.Location || report.location || "N/A", 60, nextY);

        doc.setFont('helvetica', 'bold');
        doc.text("Date:", 20, nextY + 10);
        doc.setFont('helvetica', 'normal');
        doc.text(report.Date || "N/A", 60, nextY + 10);

        if (report.Name) {
            doc.setFont('helvetica', 'bold');
            doc.text("Reporter:", 20, nextY + 20);
            doc.setFont('helvetica', 'normal');
            doc.text(report.Name, 60, nextY + 20);
        }

        // Footer
        doc.setTextColor(150, 150, 150);
        doc.setFontSize(10);
        doc.text("Generated by CityGuardian System", 20, 280);
        doc.text(new Date().toLocaleDateString(), 170, 280);

        doc.save(`CityGuardian_Report_${report.ID}.pdf`);
    };
